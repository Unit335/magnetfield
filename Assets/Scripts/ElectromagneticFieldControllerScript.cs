...
public class ElectromagneticFieldControllerScript : MonoBehaviour {
    ...
    // external sources of electric and magnetic fields
    ...
    // Keep track of the dipoles rigidbodies 
    ...
    // MagnetizableMaterials call this function in their FixedUpdate() to get forces applied
            ...
            foreach (int rId in magneticDipolePosition.Keys) {
                foreach (int mId in magneticDipolePosition[rId].Keys) {
                    ...
                    if (!magneticDipoles[mId]) {
                        // clean data from deleted dipoles
